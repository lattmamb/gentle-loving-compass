...
export function VehicleCardImage({ 
  ...
  return (
    <motion.div 
      className="relative w-full pt-[56.25%] bg-gradient-to-b from-blue-700/30 to-blue-900/50"
      whileTap={{ scale: 0.98 }}
      drag="x"
    ...
    >
      ...
      <div className="absolute top-4 left-4 z-10">
        <div className="text-sm font-medium text-blue-300">{type || "Model " + model}</div>
      </div>
      ...
      <div className="absolute bottom-4 left-0 right-0 flex justify-center gap-1 text-white/70 text-xs">
        <span>← swipe →</span>
      </div>
    </motion.div>
  );
